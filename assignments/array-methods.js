...
// Problem 1
// Runners who have donated at least $100 get a special thank-you card. Create an array (of objects) of those top fundraisers.

let topMoneyRaisers = runners.filter(runner => runner.donations >= 100);
console.log(topMoneyRaisers);


// Problem 2
// All runners that are 3XL need extra help training. List their names.

let bigRunnerNames = [];
let bigRunners = runners.filter(runner => runner.shirt_size === "3XL");
bigRunners.forEach(function (items) {
  return bigRunnerNames.push(`${items.first_name} ${items.last_name}`);
});
console.log("Here are the big runners");
console.log(bigRunners);
console.log(bigRunnerNames);


// Problem 3
// Skinix pays the membership fees for the runners in the club. They need to know how many of their employees are members.
let skinixMemberCount = runners.filter(runner => runner.company_name === "Skinix").length;

/* The .reduce way. (It doesn't work yet)
 .reduce(function(accumulator, item)) {
  return ++accumulator;
},0);
*/
console.log(`There are ${skinixMemberCount} Skinix members at the club`);
